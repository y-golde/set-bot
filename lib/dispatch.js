...
async function handleSyncLaunch({ launch, mode, agentProvider, notifyArgs }) {
  ...
  const marker = launch.agentId ? `<br><sub>agent:${agentProvider.id}:${launch.agentId}</sub>` : '';

  if (result.status === 'failed') {
    await notify(
      notifyArgs,
      `❌ ${capitalize(mode.label)} agent (<code>${agentProvider.id}</code>) failed: <code>${escapeHtml(result.statusRaw ?? 'error')}</code>${marker}${sign('angry')}`
    );
    return;
  }
  ...
  await notify(
    notifyArgs,
    `✅ <b>${capitalize(mode.label)} complete</b> (<code>${agentProvider.id}</code>)${marker}<br><br>${html}${sign('happy')}`
  );
}
