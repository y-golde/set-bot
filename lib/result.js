...
export async function postAgentResult({ itemId, agentId, mode = null, force = false, webhookBody = null, notifyEmail = null }) {
  const label =
  ...
  if (failed) {
    await notify(
      itemId,
      `❌ ${label} agent ended with status <b>${escapeHtml(status || 'unknown')}</b>. Check the Cursor agent page for details. grrr 🐶`,
      notifyEmail
    );
  ...
  const prUrl = findPrUrl(agentRecord, resultText, webhookBody);
  const prBlock = prUrl
  ...
  await notify(itemId, `${statusBanner}${prBlock}<br><br>${html}`, notifyEmail);
  return { posted: true, status, prUrl };
}

function findPrUrl(agent, text, webhookBody) {
  const direct =
    webhookBody?.target?.prUrl ??
    webhookBody?.target?.pullRequest?.url ??
    webhookBody?.prUrl ??
    agent?.target?.prUrl ??
