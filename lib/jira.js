...
export async function getIssue(issueKey) {
  // Pull the fields we care about. expand=renderedFields would give us HTML
  // but we read description as ADF and stringify it ourselves.
  return jiraFetch(`/rest/api/3/issue/${encodeURIComponent(issueKey)}?fields=summary,description,reporter,${repoFieldId()}`);
}
