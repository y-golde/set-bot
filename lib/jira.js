...
// Write a repo URL into the configured Jira custom field. Returns true on
// success, false if no field id is configured. URL-type custom fields
// accept either a bare string or `{ url, title }` — try the simple form
// first, retry with the object form on 400.
export async function setRepoFieldValue(issueKey, url) {
  const fieldId = repoFieldId();
  if (!fieldId) return false;
  try {
    await jiraFetch(`/rest/api/3/issue/${encodeURIComponent(issueKey)}`, {
      method: 'PUT',
      body: JSON.stringify({ fields: { [fieldId]: url } }),
    });
    return true;
  } catch (err) {
    ...
    await jiraFetch(`/rest/api/3/issue/${encodeURIComponent(issueKey)}`, {
      method: 'PUT',
      body: JSON.stringify({ fields: { [fieldId]: { url, title: url } } }),
    });
    return true;
  }
}
