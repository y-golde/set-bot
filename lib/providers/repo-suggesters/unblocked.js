...
// Per-attempt poll budget. We try twice (see ATTEMPTS) since Unblocked
// occasionally returns state=complete with an empty answer on a cold
// question. Two attempts × 25s leaves headroom under the 60s function cap.
...
export async function suggest({ ticketName, contextBlock, ticketUrl }) {
  ...
  for (let attempt = 1; attempt <= ATTEMPTS; attempt++) {
    ...
    try {
      ...
      console.log(`[repo-suggest:unblocked] submitted question ${questionId} (attempt ${attempt}/${ATTEMPTS})`);
      const answer = await pollAnswer({ baseUrl, token, questionId });
      if (!answer) {
        console.log(`[repo-suggest:unblocked] attempt ${attempt} returned empty answer; ${attempt < ATTEMPTS ? 'retrying' : 'giving up'}`);
        continue;
      }
      const preview = answer.length > 400 ? answer.slice(0, 400) + '…' : answer;
      console.log(`[repo-suggest:unblocked] answer for ${questionId}:`, JSON.stringify(preview));
      const parsed = parseSuggestion(answer);
      if (!parsed) {
        console.log(`[repo-suggest:unblocked] answer did not parse to a repo (likely "NONE" or wrong format)`);
      }
      return parsed;
    } catch (err) {
      console.error(`[repo-suggest:unblocked] attempt ${attempt} failed:`, err);
...
async function pollAnswer({ baseUrl, token, questionId }) {
  ...
  while (Date.now() < deadline) {
    ...
    if (body?.state === 'complete') {
      // Log the raw body once we get a terminal state so we can verify
      // the field path. Trim to keep logs readable.
      const dump = JSON.stringify(body);
      console.log(`[repo-suggest:unblocked] poll #${attempts} complete; raw body: ${dump.slice(0, 1500)}${dump.length > 1500 ? '…' : ''}`);
      // Try documented path first, then fall back to other plausible shapes.
      const answer = body.result?.answer ?? body.answer ?? body.result ?? '';
      return typeof answer === 'string' ? answer : JSON.stringify(answer);
    }
    if (body?.state && body.state !== 'processing') {
      throw new Error(`Unblocked unexpected state: ${body.state} — body: ${JSON.stringify(body).slice(0, 500)}`);
    }
  }
  console.warn(`[repo-suggest:unblocked] poll timed out after ${POLL_TIMEOUT_MS}ms (${attempts} attempts)`);
  return null;
}
