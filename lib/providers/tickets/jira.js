...
export async function getTicketContext(ticketId) {
  ...
  const cleanedComments = cleanCommentList(comments);

  // Dump the whole issue + filtered comments as JSON. Bot comments and
  // `!set` trigger lines are stripped so they don't pollute the agent's
  // view of the conversation.
  const contextBlock = JSON.stringify(
    { issue, comments: cleanedComments },
    null,
    2
  );
...
function cleanCommentList(comments) {
  const out = [];
  for (const c of comments) {
    const text = adfToText(c.body);
    const cleaned = cleanComment(text);
    if (!cleaned) continue;
    out.push({ ...c, body: cleaned, cleanedText: cleaned });
  }
  return out;
}
...
export async function setRepoUrl(ticketId, url) {
  return setRepoFieldValue(ticketId, url);
}
...
export function findRepoSuggestion(context) {
  ...
  for (const c of comments) {
    const text = adfToText(c.body);
    const m = text.match(/set-suggest:\s*(https?:\/\/\S+)/i);
    if (m) return { repoUrl: m[1].replace(/[<>"')]+$/, '') };
  }
  return null;
}
