...
// Write a repo URL into the item's link column. Returns true on success,
// false if we couldn't find a matching column on the board.
export async function setRepoUrl(itemId, url, columnTitle = 'Repositories') {
  const item = await getItemWithColumns(itemId);
  ...
  // Link columns expect { url, text } JSON. Text columns also accept it.
  const value = JSON.stringify({ url, text: url });
  const query = `
    mutation SetColumn($itemId: ID!, $boardId: ID!, $columnId: String!, $value: JSON!) {
...
async function getBoardIdForItem(itemId) {
  const data = await mondayGraphQL(
    `query GetBoard($itemId: [ID!]) { items(ids: $itemId) { board { id } } }`,
    { itemId: [String(itemId)] }
  );
  return data?.items?.[0]?.board?.id ?? null;
}
